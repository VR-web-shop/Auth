...
test('DeleteCommand should soft delete a role', async () => {
    ...
    const role = await testDB.db.Role.findOne({ where: { client_side_uuid: 'aaa-bbb-ccc' } });
    const removed = await testDB.db.RoleRemoved.findOne({ where: { role_client_side_uuid: 'aaa-bbb-ccc' } });

    expect(removed).toBeDefined();
    expect(removed.deleted_at).toBeDefined();
    expect(removed.deleted_at).not.toBeNull();
    expect(removed.role_client_side_uuid).toBe('aaa-bbb-ccc');
    expect(role).toBeDefined();
    expect(role.client_side_uuid).toBe('aaa-bbb-ccc');
});
